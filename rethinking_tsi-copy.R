...
##################################################
# 2. Random Forest for Variable Selection
##################################################
...
all_dat <- all_dat[sample(1:nrow(all_dat),20),] # For testing
##################################################
#GIS Variables
#Clean Up Data - Complete Cases
...
gis_dat_NTL <- gis_dat_NTL[sample(1:nrow(gis_dat_NTL), 20),] # For testing
gis_dat_PTL <- gis_dat_PTL[complete.cases(gis_dat_PTL),]
gis_dat_PTL <- gis_dat_PTL[sample(1:nrow(gis_dat_PTL), 20),] # For testing
...
Predict.CatAllVar <-  vector("character",length(predict.EvaluationAllVar))
...
# Start the clock!
ptm <- proc.time()
JAGS.TS.N <- jags.model('TSLogit-copy.R',data = DataList 
